...
export class DnsService {
  ...
  private async getAuthoritativeNameserverIps(
    domain: string,
  ): Promise<string[]> {
    try {
  ...
  private async getRecords<T extends any[]>(
    domain: string,
    method: () => Promise<T>,
    nsDomain?: string,
  ): Promise<T> {
  ...
  async getPrimaryStatus(domain: Domain) {
    const aRecords = await this.getARecords(
      domain.domain,
      this.getNsDomain(domain.domain),
    );
  ...
  async getChallengeStatus(domain: Domain) {
    const txtRecords = await this.getChallengeRecords(
      domain,
      this.getNsDomain(domain.domain),
    );
