...
export class AuthorizationValidator {
  ...
  private validateDeployKey(key: string) {
    const deployKeyRegex = /^-----BEGIN (OPENSSH|RSA) PRIVATE KEY-----\n([A-Za-z0-9+/=]+\n)*-----END (OPENSSH|RSA) PRIVATE KEY-----$/;
  ...
  private async validatePersonalAccessToken(token: string) {
    try {
      ...
      const scopes = scopesHeader.split(',').map((scope: string) => scope.trim());

      if (!scopes.includes('repo')) {
        throw new Error('Personal Access Token does not have the required "repo" scope to access repositories');
      }

    } catch (error) {
      throw new BadRequestException('Invalid Personal Access Token or insufficient permissions');
    }
  }
  ...
  public async validate(authorization: CreateAuthorizationDTO) {
    switch (authorization.type) {
      ...
      case AuthorizationEnum.PERSONAL_ACCESS_TOKEN:
        await this.validatePersonalAccessToken((authorization.data as PersonalAccessTokenData).token);
        break;
    }
  }
}
